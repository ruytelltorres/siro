(function ($) {
    $.fn.Tabla = function (m) {
        ...
        CargarDataTable("#" + m.tblId);

        var html = '';
        //html = '<table width="100%" data-edit="false" id="' + m.tblId + '" class="' + m.classtbl + ' tbl-siro tblcab">';
        html = '<table width="100%" data-edit="false" id="' + m.tblId + '" class="' + m.classtbl + '">';
                ...
                for (k in camp) {
                    ...
                    if (tipo.length > 0) {
                        switch (tipo[k]) {
                            ...
                            case "3": { dat = moment(dat).format("DD/MM/YYYY"); break; }
        ...
        if (m["paginacion"] != "No" && m.datos != null && m.datos.length > 0) {
            ...
            for (i in col) {
                if (anchocol.length > 0) {
                    if (eval(anchocol[i]) > 0) {
                        html += '<td>' + col[i].replace('<strong>', '').replace('</strong>', '') + '</td>';
                    }
                }
            }
            html += '<td></td></tr></tfoot>';
        ...
        function CargarDataTable(tableid) {
