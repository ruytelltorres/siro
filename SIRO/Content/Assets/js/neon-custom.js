...
; (function ($, window, undefined) {
    ...
    $(document).ready(function () {
        ...
        // Sidebar Menu Setup
        setup_sidebar_menu();
        setup_config_selection(); //ADD TORE
            ...
            $(".select2").each(function (i, el) {
                var $this = $(el),
                    opts = {
                        ...
                        placeholder: "Seleccione una opci\u00F3n", //ADD TORE
                        //theme: "classic",
                        width: 'resolve',
                    };
...
// CREATE TORE
function setup_config_selection() {
    var $menu = public_vars.$mainMenu;
    var current = new URL(window.location.href);
    var selected = null;
    var bestScore = -1;

    function normalizePath(path) {
        return path.replace(/\/+$/, '').toLowerCase().replace(/\/index$/, '');
    }

    // Submodules keep the menu selection of their parent list.
    var currentPath = normalizePath(current.pathname)
        .replace(/\/evaluacion\/(gestionevaluacion|gestion|detalle)$/, '/evaluacion/lista')
        .replace(/\/eventoperdida\/(modificar|detalle)$/, '/eventoperdida/lista')
        .replace(/\/riesgooperacional\/gestionriesgo$/, '/riesgooperacional/listagestion');

    $menu.find('li').removeClass('active select_menu');
    $menu.find('a[aria-current]').removeAttr('aria-current');

    // Match destinations at any depth, including modules with query parameters.
    $menu.find('a').each(function () {
        var $link = jQuery(this);
        if ($link.siblings('ul').length) return;
        var destination = $link.attr('data-url') || $link.attr('href');
        ...
        if (target.origin !== current.origin || normalizePath(target.pathname) !== currentPath) return;

        var matches = true;
        var score = 0;
        target.searchParams.forEach(function (value, key) {
            matches = matches && current.searchParams.get(key) === value;
            score++;
        });
    ...
    if (selected) {
        selected.attr('aria-current', 'page').parent('li').addClass('active select_menu');
        selected.parentsUntil($menu, 'ul').addClass('visible')
            .parent('li').addClass('opened active');
    }
}
//END CREATE TORE
